...
//Hilfe von Markus im Praktikum und Autausch mit Colleen Zeller//
interface Todo {
    text: string;
    isChecked: boolean;
...
function drawListToDOM(): void {
    ...
    for (let index: number = 0; index < todos1.length; index++) {
         ...
         */
        todo.innerHTML = "<span class='check " + todos1[index].isChecked + "'><i class='fas fa-check'></i></span>"
            + todos1[index].text +
...
function updateCounter(): void {
    counterTotalDOMElement.innerHTML = todos1.length + " in total";

    let counterChecked: number = 0;
    let counterOpen: number = 0;

    for (let i: number = 0; i < todos1.length; i++) {
        console.log(todos1[i].isChecked);

        if (todos1[i].isChecked == true) {
            counterChecked++;
        } else {
            counterOpen++;
        }
...
function addTodo(): void {
    ...
    if (inputDOMElement.value != "") {
        ...
        todos1.unshift({ text: inputDOMElement.value, isChecked: false });
...
function toggleCheckState(index: number): void {
    ...
    todos1[index].isChecked = !todos1[index].isChecked;
